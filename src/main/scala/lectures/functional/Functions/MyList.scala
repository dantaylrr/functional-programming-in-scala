...
object ListTest extends App {
  ...
  println(listOfIntegers.map(new Function1[Int, Int] {
    override def apply(transform: Int): Int = transform * 2
  }).toString) // Prints [2, 4, 6]

  println(listOfIntegers.filter(new Function1[Int, Boolean] {
    override def apply(elem: Int): Boolean = if (elem % 2 == 0) true else false
  }).toString) // Prints [1, 2, 3]

  println((listOfIntegers ++ anotherListOfIntegers).toString)
  println(listOfIntegers.flatmap(new Function1[Int, MyList[Int]] {
    override def apply(transform: Int): MyList[Int] = new Cons(transform, new Cons(transform + 1, Empty))
  }).toString)
